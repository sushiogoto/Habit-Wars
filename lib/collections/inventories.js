...
Meteor.methods({
  modifyItem: function(itemId, modification, characterId) {
  ...
  purchaseItem: function(itemId) {
    ...
    if(character.gold >= item.price) {
      Meteor.call('modifyItem', itemId, "add", character._id, function (error, result) {});
  ...
  // modifyCharacterStats:function() {
  //   var character = util.currentCharacter();
  //   var inventory = Inventories.findOne({characterId: character._id});
  //   var equippedItems = _.filter(inventory.items, {equipped: true});
  //   var healthBoost = _.pluck(equippedItems, "health").reduce(function( a, b ){
  //     return a + b;
  //   }, 0);
  //   var strengthBoost = _.pluck(equippedItems, "strength").reduce(function( a, b ){
  //     return a + b;
  //   }, 0);
  ...
  equipUnequipItem: function(itemId){
    ...
    var equippedItemOfSameType = characterInventory.items.filter(function (inventoryItem) {
      if (inventoryItem.type === undefined) return false;
  ...
  sellItem: function(itemId) {
    var character = Characters.findOne({userId: Meteor.user()._id});
    var characterInventory = Inventories.findOne({
      characterId: character._id
    }).items;
    var itemToBeSold = _.where(characterInventory, {_id: itemId})[0];
    var sellingPrice = itemToBeSold.price;

    if(!itemToBeSold.equipped){
      character.gold += sellingPrice;
      Items.remove({_id: itemId, random: true});
      Meteor.call('modifyItem', itemId, "remove", character._id,  function (error, result) {});
      Characters.update(character._id, {$set: {gold: character.gold}});
      return "Sold!";
    } else {
      // Display error: You cannot sell items that are currently equipped
      return "You cannot sell items that are currently equipped!";
    }
  }
});