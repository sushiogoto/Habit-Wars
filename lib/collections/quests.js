...
Meteor.methods({
  ...
  groupQuestVote: function(voteAction, questId) {
    ...
    if (quest.accepted.indexOf(character._id) < 0 && quest.declined.indexOf(character._id) < 0  && activeQuests.length < 1) {
      voteAction === "accept" ? quest.accepted.push(character._id) : quest.declined.push(character._id);
    }
  ...
  groupChosenQuest: function() {
    ...
    quests.forEach(function(quest) {
      ...
      if ((groupAcceptedRatio > 0.74 && group.members.length > 1) || (moment() > quest.startDate.add(1, 'd') && groupAcceptedRatio > 0.5)) {
      ...
      } else if(moment() > quest.startDate.add(1, 'd') && groupAcceptedRatio < 0.5) {
