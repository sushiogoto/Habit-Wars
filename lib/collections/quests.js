...
Meteor.methods({
  ...
  questAttack: function() {
    ...
    // Randomly generate items based on quest difficulty
    var randomItemReward = function(){
      ...
      var rewardItem = Items.insert({
        name:          "New Gear",
        type:          _.sample(itemTypes),
        health:        Math.floor(character.max_health * 0.3 * randomItemStatsModifier[quest.difficulty]),
        strength:      Math.floor(character.strength * 0.5 * randomItemStatsModifier[quest.difficulty]),
        intelligence:  Math.floor(character.intelligence * 0.5 * randomItemStatsModifier[quest.difficulty]),
        quantity:      1,
        price:         Math.floor(50 * (0.8 + Math.random() * 0.4))
      });
      Meteor.call('modifyItem', rewardItem, 'add', function(error, result){});
    };
